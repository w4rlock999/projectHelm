...
export function updateHeartbeat(
  id: string,
  patch: Partial<
    Pick<Heartbeat, 'name' | 'cron' | 'prompt' | 'enabled' | 'targetType' | 'targetChatId'>
  >,
): Heartbeat | null {
  ...
  if (patch.targetType !== undefined || patch.targetChatId !== undefined) {
    ...
    const targetChatId =
      patch.targetChatId !== undefined ? patch.targetChatId : existing.targetChatId;
...
const running: Set<string> =
  (globalThis as any).__helmHbRunning ?? ((globalThis as any).__helmHbRunning = new Set());
...
async function fireHeartbeat(hb: Heartbeat): Promise<void> {
  ...
  try {
    if (hb.targetType === 'chat' && hb.targetChatId) {
      ...
      if (!chat) {
        console.error(
          `[helm] heartbeat ${hb.id}: target chat ${hb.targetChatId} not found — skipping`,
        );
        return;
      }
