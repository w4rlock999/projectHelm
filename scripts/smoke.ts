...
function ensurePirateAgent() {
  ...
  if (existing) {
    console.log(`[smoke] reusing agent ${existing.id} (session=${existing.claudeSessionId ?? 'new'})`)
    return existing
  }
...
async function main() {
  ...
  const { code } = await runClaude({
    ...
    onEvent: (evt) => {
      ...
      if (evt.type === 'stream_event' && evt.event.type === 'content_block_delta') {
        ...
        console.log(
          `[smoke] done · cost=$${evt.total_cost_usd.toFixed(4)} · ${evt.duration_ms}ms`,
        )
      }
    },
    onLog: () => {
  ...
  console.log(
    `[smoke] db session after run: ${after?.claudeSessionId ?? 'null'}`,
  )
