...
async function main() {
  ...
  assert(Array.isArray(initialList), `tRPC agents.list returns an array (got ${initialList.length} agents)`)
  ...
  assert(
    claudeMdContent.includes('pirate-speak'),
    `CLAUDE.md contents reflect the system prompt`,
  )
  ...
  const psBuf = await (await import('node:child_process')).execSync(
    "ps aux | grep -E 'claude --print|claude -p' | grep -v grep | wc -l",
    { encoding: 'utf8' },
  )
  const claudeProcs = parseInt(psBuf.trim(), 10)
  assert(claudeProcs === 0, `No orphan claude processes after client disconnect (${claudeProcs} found)`)
